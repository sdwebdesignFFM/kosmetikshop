...
class RegistrationController extends Controller
{
    ...
    public function store(RegistrationRequest $registrationRequest)
    {
        ...
        \Log::info('Registration Request Data:', [
            'has_file' => $registrationRequest->hasFile('business_document'),
            'files' => $registrationRequest->allFiles(),
            'all_data' => $registrationRequest->all()
        ]);

        $customerGroup = core()->getConfigData('customer.settings.create_new_account_options.default_group');

        $businessDocumentPath = null;
        if ($registrationRequest->hasFile('business_document')) {
            // Store in private storage (not publicly accessible)
            $businessDocumentPath = $registrationRequest->file('business_document')->store('business_documents', 'local');
        }
